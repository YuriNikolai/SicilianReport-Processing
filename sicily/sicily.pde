...
SoundFile[] sfxVars = new SoundFile[10];
...
void setup() {
  ...
  println(gfxfolder);
  bg = loadImage(gfxfolder + "//" + "bg.png");
  centerpiece = loadImage(gfxfolder + "//" + "centerpiece.png");
  cloud = loadImage(gfxfolder + "//" + "cloud.png");

  //                     lampedusa    christ      clip1     clip2       clip3       clip4       marabout   worncard1  worncard2  crescentii lampcard
...
void mousePressed() {
  for (int i = dragImages.length - 1; i >= 0; i--) {//Starts from the last, loops backward so we always grab pic on top
    if (mouseButton == LEFT) {
      dragImages[i].draggingpicMousePressed(sfxVars[0]);
    }
...
void mouseReleased() { //sets 'hold' variable to false
  for (DraggingPic currentDraggingPic : dragImages) {
    currentDraggingPic.draggingpicMouseReleased(sfxVars[1]);
  }//for
